...
import type { ReferralsResponse } from '@/types/api'
...
export async function getReferrals(params: { q?: string; onboarding?: string; account?: string; page?: number; limit?: number } = {}): Promise<ReferralsResponse> {
  ...
  return getJSON<ReferralsResponse>(`/referrals${qs ? `?${qs}` : ''}`)
}
...
export async function getReferralDetail(username: string): Promise<import('@/types/api').ReferralDetailResponse> {
  return getJSON<import('@/types/api').ReferralDetailResponse>(`/referrals/${encodeURIComponent(username)}`)
}

export async function getReferralOrders(username: string, params: { page?: number; limit?: number } = {}): Promise<import('@/types/api').ReferralOrdersResponse> {
  const usp = new URLSearchParams()
  if (params.page) usp.set('page', String(params.page))
  if (params.limit) usp.set('limit', String(params.limit))
  const qs = usp.toString()
  return getJSON<import('@/types/api').ReferralOrdersResponse>(`/referrals/${encodeURIComponent(username)}/orders${qs ? `?${qs}` : ''}`)
}
